...
class FtpDirectory extends FtpEntry {
  ...
  Future<bool> exists() async {
    ...
    final response = await FtpCommand.CWD.writeAndRead(socket, [path]);
    await FtpCommand.CWD.writeAndRead(socket, [currentDirectory]);
    return response.isSuccessful;
  }

  Future<String> _currentDirectory() async {
  ...
  Future<bool> create({bool recursive = false}) async {
    final response = await FtpCommand.MKD.writeAndRead(socket, [path]);
  ...
  Future<bool> delete({bool recursive = false}) async {
    ...
    final response = await FtpCommand.RMD.writeAndRead(socket, [path]);
  ...
  Future<FtpDirectory> rename(String newName) async {
    ...
    var response = await FtpCommand.RNFR.writeAndRead(socket, [path]);
    ...
    response = await FtpCommand.RNTO.writeAndRead(socket, [
      newPath,
    ]);
    if (!response.isSuccessful) {
      throw Exception('Could not rename directory');
    }
    return FtpDirectory(path: newPath, socket: socket);
  }
  ...
  Future<FtpDirectory> move(String newPath) async {
    ...
    final newDirectory = FtpDirectory(path: newPath, socket: socket).parent;
